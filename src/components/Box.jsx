...
const Box = ({bgColor, borderColor, text, border, image}) => {
  ...
  return (
    <>
      {image ? (
        <>
        <div style={boxStyle} className={`flex flex-row justify-between border-${border} border-4 my-4 pr-8`}>
          <p className="w-[85%] text-2xl p-4 lg:text-2xl md:text-sm">{text}</p>
          <img src={image} />
        </div> 
        </>
      ) : (
